...
-- 1) Lista de livros disponíveis com autor e categoria (JOIN + ORDER BY)
...
-- 2) Empréstimos atrasados (WHERE + JOIN)
...
-- 3) Quantidade de exemplares e disponíveis por livro (GROUP BY + COUNT)
...
-- 4) Usuários com multas pendentes (JOIN + WHERE)
...
-- 5) Buscar livros por termo no título (LIKE) com LIMIT
SELECT id, titulo, isbn FROM livro
WHERE titulo LIKE '%Hora%' OR titulo LIKE '%Harry%'
ORDER BY titulo
LIMIT 10;
